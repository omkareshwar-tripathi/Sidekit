...
namespace SpeakType.Tests.Logging;
...
public sealed class FileLogSinkTests : IDisposable
{
    ...
    {
        var path = Path.Combine(_tempDir, "logs", "speaktype.log");
    ...
    {
        var path = Path.Combine(_tempDir, "speaktype.log");
        var sink = new FileLogSink(path);
    ...
    public void Write_is_best_effort_and_swallows_io_failure()
    {
        ...
        var sink = new FileLogSink(Path.Combine(blocker, "speaktype.log"));
