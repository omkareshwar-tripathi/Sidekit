...
namespace SpeakType.Tests.Paste;
...
public sealed class ClipboardPasteServiceTests
{
    ...
    /// so tests can assert the save→set→target?→paste→delay→restore sequence.
    /// </summary>
    ...
    [Fact]
    public void Confirmed_target_runs_the_full_sequence_in_order()
    {
        _clipboard.CurrentText = "ORIGINAL";
        _clipboard.TargetConfirmed = true;

        var outcome = CreateSut().Paste("OURTEXT");

        Assert.Equal(PasteOutcome.Pasted, outcome);
        Assert.Equal(
            new[] { "GetText", "SetText:OURTEXT", "HasEditableTarget", "SendPaste", "Delay", "SetText:ORIGINAL" },
            _clipboard.Ops);
    }
    ...
    [Fact]
    public void No_editable_target_leaves_our_text_on_the_clipboard()
    {
        _clipboard.CurrentText = "ORIGINAL";
        _clipboard.TargetConfirmed = false;
        ...
        Assert.DoesNotContain("SendPaste", _clipboard.Ops);
        Assert.DoesNotContain("Delay", _clipboard.Ops);
        Assert.DoesNotContain("SetText:ORIGINAL", _clipboard.Ops); // nothing restored
