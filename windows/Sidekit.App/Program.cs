...
internal static class Program
{
    ...
    private static void Main()
    {
        ...
        if (!createdNew)
        {
            ...
            timer.Start();
            notice.ShowBalloonTip(3000, AppInfo.Name, "Sidekit is already running", ToolTipIcon.Info);
            Application.Run();
            return;
        }
        ...
        // Relocate any per-user data left by the pre-rebrand "SpeakType" app into the Sidekit
        // folders, once, before the first settings/model/log path is read below.
        DataMigration.MigrateLegacyAppData(AppInfo.Name);
        ...
        // LAST — after the model loads — so it stays inert until Sidekit is actually ready (spec Feature 2).
