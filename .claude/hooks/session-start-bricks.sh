...
#
# SpeakType uses a single BRICKS.md handoff file (see CLAUDE.md §2b) instead of
...
#   SpeakType-v1-spec.md  — the complete, decision-resolved spec
...
PREFIX="=== SpeakType Build Status (live, from BRICKS.md) ===

Project follows a strict brick-by-brick loop (CLAUDE.md §2a). Read this handoff log, then read the relevant SpeakType-v1-spec.md section and the matching TESTING.md M# items before acting. Work only the current brick — the top unchecked item under 'Next up' — never work ahead. Update BRICKS.md the moment a brick is reviewed + tested + verified (§2b).
...
FALLBACK="=== SpeakType Build Status ===

No BRICKS.md found yet — the brick-by-brick build hasn't started, or the file was removed.

Recovery: read CLAUDE.md and SpeakType-v1-spec.md for context, then re-establish BRICKS.md from the spec.
=== End Build Status ==="
