...
### Brick INST-1 — per-user Inno Setup installer (2026-06-04)
- **What:** Wraps the single-file exe in `SpeakTypeSetup.exe` so the app installs like real software: **per-user** (no admin/UAC) to `%LOCALAPPDATA%\Programs\SpeakType`, with a **Start Menu** entry, an optional **Desktop** shortcut, and a clean **uninstall** (Add/Remove Programs). Resolves the long-deferred "real installer" backlog item (lightweight option a wasn't taken — went straight to a proper installer).
- **Files:** `installer/SpeakType.iss` (new, Inno Setup 6 script); `.github/workflows/ci.yml` (choco install innosetup → ISCC compile → upload `SpeakTypeSetup` artifact); `.gitignore` (`installer/output/`).
- **Verified:** Windows **CI green (PR #4, run 26908135701)** — Inno Setup installs, the installer compiles, **`SpeakTypeSetup.exe` = 195.3 MB** uploaded. **M-test pending:** run the setup on a real box → installs without admin, shortcuts work, uninstall removes it.
- **Notes / decisions:** Per-user (`PrivilegesRequired=lowest`) chosen by the user — friendliest for a personal tray app. AppId GUID `A14E0322-…` is fixed so upgrades replace cleanly. Still **unsigned** → SmartScreen warns on the setup; only a code-signing cert removes that (separate, paid, optional). **Known follow-up (regression from CE-5c):** baking in the speech model makes the Whisper first-run gate skip, so `ApplyAutostart(true)`/"Ready!" no longer fire on a fresh install — "Start with Windows" now defaults OFF until toggled in Settings. Decide whether to restore default-on autostart via an independent first-run marker.
