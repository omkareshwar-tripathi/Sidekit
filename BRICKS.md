...
- [ ] **SHELF-THUMBS** — real **QuickLook thumbnails** (files/images) / content previews (text → first line) in the tiles, replacing the kind-glyph placeholder; the glyph stays the fallback when bytes are missing. **← current** (App-layer — manual)
  - Skill: none (verification-before-completion)
- [ ] **SHELF-DRAG-OUT** — multi-select drag **out** of the panel into any app/folder via file promises; drag-out never removes the item (reuse-safe, spec §4). (App-layer — manual)
  - Skill: none (verification-before-completion)
- [ ] **SHELF-ITEM-ACTIONS** — per-item **⋯** (Copy to clipboard / Reveal in Finder) + the footer **Save-all-to…** folder picker (writes every item's bytes into the chosen folder). (App-layer — manual)
  - Skill: none (verification-before-completion)
...
- [x] **SHELF-DROP-IN — drag files / folders / text / images INTO the shelf (2026-06-10).** Second slice of the split SHELF-DROP — the panel now actually receives drops. New pure **`ShelfPayloadSource`** (`.file(URL)` / `.text(String)` / `.image(Data)`) + **`StoredPayload`** value types (Core, Ports.swift) and a copy-in method **`store(_:) throws -> StoredPayload`** on the `ShelfPayloadStore` port (Noop throws `.unsupported` → a drop against an unwired store stages nothing). **`ShelfStore.add(from:)`** (Core) copies the bytes via the port **then** stages the returned metadata — the copy-then-record ordering (no dangling index entry on a failed copy) lives in the pure core, mirroring the delete path. New adapter copy-in **`FileSystemShelfPayloadStore.store`** writes into a fresh `…/Shelf/<uuid>/<name>` folder (file/folder → `copyItem`; text → `snippet.txt`; image → `image.png`), honoring the `<uuid>` layout `delete` already removes; `byteSize(of:)` sums a file or whole folder for the footer. **`ShelfModel.acceptDrop`** copies + records (logs a swallowed failure via `Diag`). **`ShelfView`** gains `.onDrop(of: [.fileURL, .image, .text])` with an accent drop-target highlight; the off-main `NSItemProvider` decode prefers **file → image → text** (so a Finder image-file shelves as the file) and a **non-file URL (e.g. a dragged browser link) is shelved as a text snippet** rather than silently vanishing. **Removed the SHELF-UI `#if DEBUG` sample seeder** (`seedSamples()` + header ＋ button) — real drops exist now. **Files:** `Sources/SpeakTypeCore/{Ports,ShelfStore}.swift`, `Sources/SpeakTypeApp/Adapters/FileSystemShelfPayloadStore.swift`, `Sources/SpeakTypeApp/{ShelfModel,ShelfView}.swift`, `Tests/SpeakTypeCoreTests/{Fakes,ShelfStoreTests}.swift` (+2 `add(from:)` tests). **Verified (TDD):** tests first → RED (`add(from:)` missing) → impl → **GREEN: core 90/90** (+2); `swift build` clean; `/simplify` (unified `byteSize` to one path; added off-main-copy TODO; logged the swallowed failure) + `/code-review` low (fixed the browser-link silent-drop; wiring/conformers/data-race all clean) applied. **Manual (Mac) — pending user:** open Shelf → drag a file, a folder, a text selection, an image, and a browser link onto it → each appears as a tile (kind glyph for now) with the right name; footer count/size updates; the card shows an accent border while dragging; quit + relaunch → items persist; remove/Clear-all deletes the bytes. **Notes:** Two silent-drop edges (un-encodable image; security-scoped URLs when sandboxed) parked as **SHELF-DROP-EDGES**. Thumbnails/Save-all/⋯ still later bricks.
  - **FIX ROUND — drag-in actually works now (2026-06-10, user-verified ✅).** Manual test exposed the first cut as broken: a text-conforming file from Finder (e.g. `fetch-model.sh`) shelved as a **text snippet of its contents** (the shebang), and a folder shelved as a **`file:///…` text snippet**. Root cause (found via a 9-agent investigation workflow that ran `swift` against the type system): SwiftUI's declared `.text` drop type **coerces a text-conforming file to its content UTI and strips `public.file-url`**, so `suggestedName`/`isFileURL` routing failed (`suggestedName` is `nil` for all Finder drags). **Fix — routing rewritten to key off the provider's *registered types*** (`ShelfView.load`): (1) `hasItemConformingToTypeIdentifier(.fileURL)` → copy the real on-disk item via `loadObject(URL)` (Finder files **and folders**); (2) else a registered *content* type that is non-URL, non-image, and within the text family only **source code** (`conforms(to: .sourceCode)`) → materialize via `loadFileRepresentation` — this routes `.sh`/`.py`/`.swift` to a file while a rich/plain-text **selection** (`.rtf`/`.html`/plain — caught by an adversarial verifier) correctly falls through to text; (3) image; (4) web-URL → text; (5) text. **Architecture change:** `ShelfStore.add(from:)` → **`record(_ payload:)`** (Core, metadata-only); copy-then-record orchestration moved to a new **`nonisolated ShelfModel.ingest`** that copies bytes **off the main actor** inside the drop callback (a `loadFileRepresentation` temp URL is valid only there) and hops to `@MainActor` only to record — this also **resolves the main-actor-copy concern** the earlier review flagged. **Tile names now word-wrap** to 2 lines (`lineLimit(2)` + reserved `nameHeight`) per user request. **Files:** `ShelfView.swift` (load rewrite + `.onDrop` `[model]` capture + diagnostics + word-wrap), `ShelfModel.swift` (`ingest`, holds `payloadStore`), `ShelfStore.swift` (`record`), `FileSystemShelfPayloadStore.swift` (off-main note), `Fakes.swift`/`ShelfStoreTests.swift` (`add(from:)`×2 tests → 1 `record` test). **Verified:** **89/89**, `swift build` clean (no concurrency warnings); 2 focused correctness reviews clean (off-main ordering / actor isolation / no dead code); **user dragged `.sh` + folder + others from Finder → all correct ✅**. **Diagnostics:** `ShelfView.load` logs `types=/hasFileURL=/content=/branch=` to `~/Library/Logs/SpeakType.log` (opt-in via `SpeakTypeDebug`) — kept for future drag-source debugging. **Gotcha for the next session:** never re-add `.text`-only routing or `suggestedName` gating — both fail for Finder text-files; the `.sourceCode` discriminator is load-bearing.
- [x] **SHELF-PAYLOAD — payload-byte store + "bytes never outlive the index" (2026-06-10).** First slice of the split SHELF-DROP. New pure **`ShelfPayloadStore`** port (`delete(_ items:)`) + **`NoopShelfPayloadStore`** default (Ports.swift). **`ShelfStore`** gained an injected `payloads` (defaults to noop, so existing call sites/tests are unchanged) and now calls `payloads.delete(...)` on **remove / clearAll / pruneExpired** — so an item's bytes are always cleaned when its index entry leaves. New adapter **`FileSystemShelfPayloadStore`** (App) deletes the per-item folder (`…/Application Support/SpeakType/Shelf/<uuid>/`, the first path component of `storedRelativePath`), swallowing `fileNoSuchFile` (a never-written/debug-seeded item is fine) and logging other errors; wired into `ShelfModel`'s production init. **Copy-in (writing dropped bytes) is deferred to SHELF-DROP-IN** — bytes only arrive on a drop. **Files:** `Sources/SpeakTypeCore/{Ports,ShelfStore}.swift`, `Sources/SpeakTypeApp/Adapters/FileSystemShelfPayloadStore.swift` (new), `Sources/SpeakTypeApp/ShelfModel.swift`, `Tests/SpeakTypeCoreTests/{Fakes,ShelfStoreTests}.swift` (+`FakeShelfPayloadStore`, +4 tests). **Verified (TDD):** tests first → RED (`ShelfPayloadStore` missing) → impl → **GREEN: core 88/88** (+4); `swift build` clean (adapter links); `/code-review` + cleanup review both clean (path-resolution dup under rule-of-three; noop+defaulted-param mirrors `NotesPersisting`; delete-after-save ordering correct). **Notes:** fully auto-verified (Core+adapter, no manual Mac step). delete runs **after** `persistence.save` — a crash between leaves orphan bytes (safe) rather than a dangling index entry. Adapter assumes drop-in stores bytes as `<uuid>/<file>`; SHELF-DROP-IN must honor that layout.
