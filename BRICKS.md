...
- [ ] **SHELF-SETTINGS** — TTL configuration + store-size readout/management in Settings.
  - Skill: none (verification-before-completion)
- [ ] **SHELF-POLISH** — slide animations (Reduce-Motion aware), empty state, glass styling pass. **Fold in (revealed by SHELF-DROP-IN review):** move the payload copy + recursive size walk **off the main actor with progress** (today `FileSystemShelfPayloadStore.store` runs on `@MainActor` via `acceptDrop` — fine for small drops, a large folder would freeze the UI; spec §7 risk 2 — `TODO` marker is in the file).
  - Skill: none (verification-before-completion)
