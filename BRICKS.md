...
- [ ] **Brick 6b — HTTP model downloader (`IModelDownloader`).** Real `HttpClient` streaming download to a destination path with `IProgress<double>` (received/total) reporting; honours `CancellationToken`; one attempt per call (the Brick 6a `ModelStore` owns the retry loop + size/SHA256 verify + atomic install). Cross-platform (BCL `System.Net.Http`) — Mac-testable with a mocked `HttpMessageHandler`; the real network download is M2-gated. Lives in `SpeakType.Core` (like `JsonSettingsStore`).
  - Skill: dotnet-best-practices, dotnet-xunit, run-tests
  - Verify: unit — streams body to the path + reports progress against a fake `HttpMessageHandler` (no real network); cancellation propagates. Manual — **M2** (real download of base.en + model switch + a failure/retry).
