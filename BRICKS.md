...
- [ ] **UI-8b — Main window UI (sidebar + editor).** SwiftUI `NavigationSplitView`: left sidebar = notes list (title/preview/relative time) + "New note"; right = a plain-text editor bound to the active note (write-back via `NotesModel`, incl. a new tested `NotesStore.setBody`). Glass styling, selection cross-fade. Reads `NotesModel` (built in UI-8a). _(Split from UI-8; the observable model + debounced/flushed persistence shipped in UI-8a.)_
