...
- **CE-4d — App glue: CODE-COMPLETE, PENDING WINDOWS VERIFICATION.** Cannot compile on macOS (net8.0-windows WinForms — `dotnet build` fails at the WindowsDesktop SDK import, never reaching the C#). Restore is clean (the new Onnx reference + transitive ORT/Tokenizers packages resolve). **What was done** (4 files, committed): (1) `SpeakType.App.csproj` references `SpeakType.Onnx`. (2) `Program.cs` — `CoEditModelStore` constructed; **required** first-run download gate (chose "strictly required" like Whisper — cancel/fail → exit); `OnnxCoEditModel` + `CoEditPolisher` built once at startup (try/catch → null polisher = fail-open), injected as the new last arg of `DictationOrchestrator`, disposed last with `swappable`. (3) `SettingsForm` — "Improve text (CoEdIT)" toggle bound to `CoEditPolishing`. (4) `WelcomeForm` refactored to drive a `Func<IProgress<double>, CancellationToken, Task>` delegate (4th call site = CoEdIT) so the CoEdIT store's `EnsureAsync(progress, ct)` reuses the progress+Retry UI; the `IModelStore` constructor still works via a static `DownloadFor` adapter.
  - **TO VERIFY (Windows laptop or CI):** (a) the App **compiles** on Windows; (b) **single-file win-x64 publish** bundles the ORT + Tokenizers.DotNet **win-x64 natives** AND `assets/tokenizer.json` beside/inside the exe (self-extract) — the #1 risk, since the publish is `IncludeNativeLibrariesForSelfExtract`; (c) first run downloads + installs CoEdIT and **polishing actually happens** end-to-end; (d) the Settings toggle flips it live. **Realistic path: push `feat/coedit-polish` → the Windows CI job catches (a)/(b).**
