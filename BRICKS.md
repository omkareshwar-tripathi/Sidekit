...
- [ ] **MANUAL VERIFY Stage A (user, on Mac).** Run `SpeakType.app`: (1) mic prompt appears on launch; (2) grant Accessibility (menu warns if missing); (3) hold Fn in TextEdit → menu shows "Recording…" → release → pastes "speaktype works — captured N samples"; (4) quick Fn tap (<300 ms) does nothing; (5) Fn+another key aborts. If all pass, mark MAC-3-adapter/MAC-4/MAC-5 fully done and proceed to MAC-6.
- [ ] **MAC-6 — Whisper + wiring.** Add WhisperKit dependency; `WhisperKitTranscriber` (actor, lazy model load) replacing `StubTranscriber` in `AppController`. Confirm the transcribe API against the resolved WhisperKit version. Verify: manual — hold Fn, speak, release → real text in TextEdit. Skill: none.
