...
- [ ] **Brick 14h — Live model switch.** Wire the Settings model dropdown so changing the model actually downloads + swaps it at runtime (the last unwired Settings action — split out of 14e per §2a). Building blocks exist: `SwappableTranscriber` (14g), `IModelStore.EnsureAsync(name, IProgress<double>?, ct)`, `WhisperTranscriber(modelPath)` (IDisposable), and `SettingsForm.ModelChangeRequested` (already raised; currently **unwired** → the dropdown persists `settings.ModelSize` to disk but the live session keeps the old model).
  - **Honor the persisted model at startup (bug to fix here):** `Program.cs` currently loads `modelName = AppSettings.DefaultModelSize` (always `base.en`), ignoring `settings.ModelSize`. Load the persisted model instead (falling back to the default) so a chosen model survives restart.
  - **Wrap + inject:** wrap the loaded transcriber in a `SwappableTranscriber` in `Program.cs` and inject *that* into the orchestrator.
  - **On `ModelChangeRequested`:** download + verify the new model with a progress UI (off the UI thread; the old model keeps working during the download), then on success build a new `WhisperTranscriber` and call `swappable.Swap(...)` **on the UI thread** (per its threading contract; `Swap` disposes the old one). On failure (download/verify error): keep the old model active, surface a "Download failed — Retry" error, and **roll back** `settings.ModelSize` + the dropdown selection to the still-active model.
  - Skill: dotnet-best-practices, run-tests
  - Verify: manual — **M2** (switch to `small.en` → downloads with progress, old model usable during download, swaps when verified; kill network mid-download → error, previous model still active).
