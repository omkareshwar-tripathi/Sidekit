...
- [ ] **SHELF-DROP-IN** — drag files / folders / text / images **into** the panel (SwiftUI `.dropDestination` / `NSItemProvider`); copy the bytes via `ShelfPayloadStore` into `…/Shelf/<uuid>/` and add the item. **Removes the SHELF-UI `#if DEBUG` sample seeder** (real drops now exist). **← current** (App-layer — needs manual Mac verification)
  - Skill: none (verification-before-completion)
- [ ] **SHELF-THUMBS** — real **QuickLook thumbnails** (files/images) / content previews (text → first line) in the tiles, replacing the kind-glyph placeholder; the glyph stays the fallback when bytes are missing. (App-layer — manual)
  - Skill: none (verification-before-completion)
