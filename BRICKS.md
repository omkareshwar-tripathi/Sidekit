...
- [ ] **UI-2 — Core: `Note` + `NotesStore`.** Pure model + in-memory store: `newNote`, `append(text:to:)` (spacing rules), title = first non-empty line, newest-updated-first ordering, delete/select. Unit-tested, no I/O.
  - Skill: none (Swift) · TDD
