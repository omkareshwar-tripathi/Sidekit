...
- **Next: CE-4d — App glue (Windows-only, verify on laptop/CI).** Cannot compile on macOS (net8.0-windows WinForms). Four mechanical changes against patterns mapped in this session: **(1)** `SpeakType.App.csproj` → add `ProjectReference` to `SpeakType.Onnx` (flows ONNX Runtime + Tokenizers.DotNet natives + `assets/tokenizer.json` content into the App output; **watch the single-file win-x64 self-extract publish includes the ORT + tokenizer win-x64 natives**). **(2)** `SettingsForm` → one `AddRow(... "Improve text (CoEdIT)", MakeToggle(_settings.CoEditPolishing, v => _settings.CoEditPolishing = v))` after "Remove filler words" (orchestrator reads the flag live → applies without restart). **(3)** `Program.cs` composition → construct `CoEditModelStore`; if `GetInstalledModelDirectory()` non-null, `new OnnxCoEditModel(enc, dec)` + `new CoEditPolisher(new CoEditTokenizer(CoEditTokenizer.DefaultTokenizerPath), model)` and pass `polisher` as the new last arg of `DictationOrchestrator` (wrap construction in try/catch → null polisher = fail-open). **`OnnxCoEditModel` is IDisposable holding ~2.4 GB of sessions — dispose it last, alongside `swappable.Dispose()`.** **(4)** First-run CoEdIT download — see OPEN DECISION below. To reuse the progress modal, refactor `WelcomeForm` to take a `Func<IProgress<double>, CancellationToken, Task> download` + a status string instead of `IModelStore`+`modelName` (3 call sites: first-run, model-switch, corrupt-redownload), so the CoEdIT store's `EnsureAsync(progress, ct)` (different signature, not `IModelStore`) can share it.
  - **OPEN DECISION (ask the user):** how the 2.4 GB CoEdIT download behaves on first run — (A) blocking modal but **non-fatal** (app proceeds unpolished if it fails/cancels; retries next launch) [recommended, matches the fail-open architecture]; (B) strictly required like Whisper (fail → exit); (C) background/lazy (app usable immediately, polish switches on when the model finishes). Architecture is already fail-open (optional polisher), so (B) contradicts it; (C) is best-UX but most code.
