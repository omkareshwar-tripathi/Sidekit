...
- [ ] **MAC-3-adapter — `MacClipboard` (hardware batch).** Native `SystemClipboard` adapter: `NSPasteboard` get/set/clear + `CGEvent` ⌘V `sendPaste()`. The clipboard-safe *sequence* (`ClipboardSafePaste`) already shipped + unit-tested in MAC-3-core; this is the native primitives only. Verify: manual paste into TextEdit (needs Accessibility). Skill: none.
