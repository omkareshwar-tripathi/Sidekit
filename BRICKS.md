...
_**UI redesign complete** — UI-1…UI-11 all shipped (floating pill + scratchpad). All remaining items are pending the user's manual visual pass on the signed `.app` (see each Done entry's "Manual (Mac)" note). No queued Mac bricks._
