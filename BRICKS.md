...
- **Verified:** The new Core `ModelCatalog.Resolve` is cross-platform → `dotnet test SpeakType.Tests/...` **176/176 pass** (4 new: known name unchanged; case-insensitive match; unknown → default; null → default). The App wiring is Windows-only → Windows x64 **CI green** (run <CI_RUN_ID>). **Manual M2 (switch to `small.en` → progress, old model usable during download, swaps when verified; kill network mid-download → error + previous model still active + dropdown reverts) deferred to the laptop.**
- **Notes / decisions:**
