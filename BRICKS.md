...
- [ ] **MAC-2b — Full cleaner + Settings (split off MAC-2).** Port the full 136-LOC C# `TranscriptCleaner` filler-removal (always-filler list, `i`→`I`, leading/trailing fillers, capitalization) behind the existing `TranscriptCleaner.clean`, gated by a `Settings.fillerRemoval` flag; add a minimal `Settings` struct. Mirror `TranscriptCleanerTests` from C#. Pure, fully unit-testable. Skill: test-driven-development.
