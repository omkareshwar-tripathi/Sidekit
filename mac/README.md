...
Sidekit's native macOS app — an always-on-top surface with on-device dictation (**SpeakType**), a **Shelf**, and a **Mirror**. For dictation, **hold the 🌐 (Fn) key, speak, release** — your words are transcribed on-device and pasted into whatever app you're typing in. No network, no account, no cloud: transcription runs on the Apple Neural Engine via [WhisperKit](https://github.com/argmaxinc/WhisperKit), and the speech model is bundled into the app.

The Windows app (`windows/`) is a separate C# codebase that mirrors this one; they evolve independently.
...
Sidekit is a menu-bar-only app (no Dock icon — look for the mic icon in the menu bar). On first
...
2. **Accessibility** — needed to read the global 🌐 key and to send the ⌘V paste keystroke.
   The app opens System Settings and adds itself to the list; flip **Sidekit** on under
...
```sh
tccutil reset Accessibility com.sidekit.mac
```

Then relaunch and toggle Sidekit on under Accessibility again.
...
```sh
defaults write com.sidekit.mac SidekitDebug -bool YES   # then relaunch
tail -f ~/Library/Logs/Sidekit.log
defaults delete com.sidekit.mac SidekitDebug            # turn off again
```

(Running the binary directly from a terminal? `SIDEKIT_DEBUG=1` works too.)
...
lives in `SidekitCore` and is fully unit-tested with fakes — no hardware needed.
...
mac/
  Sources/
    SidekitCore/        pure, tested logic (ports + DictationCoordinator + cleaner)
    SidekitApp/         SwiftUI MenuBarExtra + native adapters (mic, Fn key, clipboard, WhisperKit)
